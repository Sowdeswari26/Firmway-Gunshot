...
#define LED_PIN        2     // Built-in LED (optional)
...
void setup() {
  ...
  // OLED init
  if (!display.begin(SSD1306_SWITCHCAPVCC, 0x3C)) {  // 0x3C = common address
...
void loop() {
  ...
  if (packetSize) {
    ...
    if (packet.startsWith("DETECT:")) {
      processAndDisplay(packet);
    }
  }
}

// ── Parse & Display on OLED + Serial ──────────────────────
void processAndDisplay(String pkt) {
  // Remove "DETECT:"
  pkt = pkt.substring(7);
  ...
  // Print full parsed info to Serial Monitor
  Serial.println("--- THREAT ALERT ---");
...
// ── Vibration & LED alert (sharp pulses) ─────────────────
void vibrateAndBlink(int times) {
